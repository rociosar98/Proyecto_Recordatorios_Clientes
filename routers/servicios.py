...
@servicios_router.get('/servicios', tags=['Servicios'], response_model=List[ServicioRespuesta], status_code=status.HTTP_200_OK, dependencies=[Depends(JWTBearer())])
...
@servicios_router.post('/servicios', tags=['Servicios'], response_model=dict, status_code=201, dependencies=[Depends(JWTBearer())])
...
@servicios_router.put('/servicios/{id}', tags=['Servicios'], response_model=dict, status_code=200, dependencies=[Depends(JWTBearer())])
...
@servicios_router.post('/servicios_clientes', tags=['Servicios por Cliente'], response_model=dict, status_code=201, dependencies=[Depends(JWTBearer())])
...
# listar todos los servicios asignados
@servicios_router.get('/servicios_clientes', tags=['Servicios por Cliente'], response_model=List[ServicioAsignado], status_code=200, dependencies=[Depends(JWTBearer())])
def listar_servicios_asignados(db: Session = Depends(get_database_session)):
    return ServiciosService(db).get_servicios_asignados()


# listar los servicios por vencer
@servicios_router.get('/servicios_clientes/vencimientos', tags=["Servicios por Cliente"], response_model=List[ServiciosCliente], status_code=200, dependencies=[Depends(JWTBearer())])
...
# listar los servicios de un cliente especifico
@servicios_router.get("/clientes/{cliente_id}/servicios", tags=["Servicios por Cliente"] ,response_model=List[ServicioAsignadoDetalle], status_code=200, dependencies=[Depends(JWTBearer())])
